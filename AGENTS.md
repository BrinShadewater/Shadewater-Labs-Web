# ShadewaterLabs Web (all agents)

<!-- Canonical since 2026-08-06 (roles swapped: this content lived in CLAUDE.md, which
     is now an import of this file). Edit here, keep it agent-neutral. -->
...
read order) come from the workspace chain — Codex's global `AGENTS.md` and the workspace
root files — and are not repeated here.
...
## Worktrees: enumerate before assuming state

Codex worktrees for this repo live under `~\.config\superpowers\worktrees\ShadewaterLabs-Web\`
on `codex/*` branches, outside the project tree, so a directory scan will not show them. Run
`git worktree list` before assuming the repo state is just what you can see, and check
`git remote -v` rather than inferring from upstream tracking.
